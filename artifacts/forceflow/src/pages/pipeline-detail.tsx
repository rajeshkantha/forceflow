...
export default function PipelineDetailPage() {
  ...
  const { data: run, isLoading } = useGetPipelineRun(runId || "", { query: { enabled: !!runId } });
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex justify-between items-start">
             <h1 className="text-2xl font-bold tracking-tight">
               {run?.commitMessage || <Skeleton className="h-8 w-64" />}
             </h1>
             {isLoading ? <Skeleton className="h-8 w-24" /> : run && getStatusBadge(run.status)}
          </div>
          <div className="flex items-center gap-4 text-sm text-muted-foreground">
             <div className="flex items-center gap-1"><Clock className="h-4 w-4" /> {formatDuration(run?.durationSeconds)}</div>
          ...
          <CardContent>
            {isLoading ? (
              ...
              <div className="space-y-0 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-border">
                {run?.stages && Object.entries(run.stages).map(([stageName, stageData]: [string, any], index) => {
                   let Icon = CircleDashed;
                   let iconClass = "text-muted-foreground bg-background";
                   
                   if (stageData.status === "passed") {
                     Icon = Check;
                     ...
                     Icon = Loader2;
                     iconClass = "text-blue-500 bg-blue-500/10 border-blue-500/20 animate-spin";
                   }
                    ...
                    <div key={stageName} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active py-4">
                      ...
                      <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded border border-border bg-card shadow">
                        <div className="flex items-center justify-between space-x-2 mb-1">
                          ...
                          <div className="text-xs font-medium text-muted-foreground">{stageData.durationSeconds ? `${stageData.durationSeconds}s` : ''}</div>
                        </div>
                        <div className="text-sm text-muted-foreground">
                          Status: <span className="capitalize">{stageData.status}</span>
                        </div>
